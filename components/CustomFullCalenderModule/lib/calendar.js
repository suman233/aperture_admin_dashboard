...
export default class Calendar {
  ...
  renderDays() {
    ...
    const monday = this._getMonday(this.getDate());
    for (let i = 0; i < 7; i++) {
  ...
  renderCalendarHours() {
    ...
    for (let i = 0; i < calendarRows.length; i++) {
      ...
      const hour = `${`${parseInt(calendarRow.dataset.hour) % 24}`.padStart(
        2,
        "0"
      )}:00`;
  ...
  _addClicklistenerToEvent(eventDiv) {
    eventDiv.addEventListener("click", () => {
      const { eventId } = eventDiv.dataset;
      ...
      tooltip.style.left = `${
        position.left + window.scrollX + this._getCellHeightAsNumber()
      }px`;
      ...
      eventTime.innerText = this._generateDateFromDateToString(
        event.dateFrom,
        event.dateTo
      );
  ...
  _setEventStartEndSize(event) {
    ...
    if (this._isThisWeek(event.dateFrom)) {
      ...
      if (eventDiv) {
        const marginTop = startMinute
          ? (this._getCellHeightAsNumber() / 60) * startMinute
          : 0;
        eventDiv.style.height = `${
          this._getCellHeightAsNumber() - marginTop
        }px`;
    ...
    if (this._isThisWeek(event.dateTo)) {
      ...
      const eventDiv = cell.querySelector(`[data-event-id="${event.id}"]`);
      if (eventDiv) {
        const marginBottom = endMinute
          ? (this._getCellHeightAsNumber() / 60) * (60 - endMinute)
          : 0;
        if (eventDiv.style.height) {
          eventDiv.style.height = `calc(${eventDiv.style.height} - ${marginBottom}px)`;
        } else {
          eventDiv.style.height = `${
            this._getCellHeightAsNumber() - marginBottom
          }px`;
        }
      }
    }
  ...
  _getCellHeightAsNumber() {
    return parseFloat(
      getComputedStyle(document.documentElement)
        .getPropertyValue("--calendar-cell-height")
        .replace("px", "")
    );
  }
  ...
  _getEventName(event, timeSlots, index) {
    ...
    const previousSlotDay = timeSlots[index - 1]
      ? timeSlots[index - 1].day
      : null;
  ...
  _getHours(dateFrom, dateTo) {
    ...
    const currentDateFrom =
      dateFrom.getTime() <= this._getMonday(this.getDate()).getTime()
        ? this._getMonday(this.getDate())
        : dateFrom;
    const currentDateTo =
      dateTo.getTime() >= this._getSunday(this.getDate()).getTime()
        ? this._getSunday(this.getDate())
        : dateTo;
  ...
  _createCalendarBodyColumns(row) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      column.style.height = `${this._getCellHeightAsNumber()}px`;
      column.innerHTML = `<td>xxx</td>`;
  ...
  _isExistInThisWeek(dateFrom, dateTo) {
    this.counter += 1;
    return (
      this._getMonday(this.getDate()).getTime() >=
        this._getMonday(dateFrom).getTime() &&
      this._getSunday(this.getDate()).getTime() <=
        this._getSunday(dateTo).getTime()
    );
  }
